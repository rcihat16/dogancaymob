        ...
        snapshot.forEach(doc => {
            ...
            let actionButtons = `
                <div style="display: flex; gap: 8px; margin-top: 10px; justify-content: flex-end; border-top: 1px dashed var(--border); padding-top: 10px;">
                    ${status !== 'approved' ? `
                        <button class="btn" onclick="guncelleYorumDurumu('${doc.id}', 'approved')" style="padding: 4px 10px; font-size: 11px; background-color: var(--success); color: white; border: none; border-radius: 4px; cursor: pointer; transition: opacity 0.2s;">
                            ✓ Yayınla
                        </button>
                    ` : ''}
                    ${status !== 'hidden' ? `
                        <button class="btn" onclick="guncelleYorumDurumu('${doc.id}', 'hidden')" style="padding: 4px 10px; font-size: 11px; background-color: var(--danger); color: white; border: none; border-radius: 4px; cursor: pointer; transition: opacity 0.2s;">
                            ❌ Gizle
                        </button>
                    ` : ''}
                </div>
...
function guncelleYorumDurumu(docId, status) {
    db.collection("reviews").doc(docId).update({
        ...
        loadLoginReviews(); // refresh public testimonials feed
    }).catch(err => {
...
window.guncelleYorumDurumu = guncelleYorumDurumu;
